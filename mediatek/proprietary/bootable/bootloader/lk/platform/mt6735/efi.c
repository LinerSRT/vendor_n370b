...
static u64 get_gpt_header_last_usable_lba()
{
	...
	return (last_lba(dev->blkdev->part_user)-(GPT_PART_SIZE/dev->blkdev->blksz));
}
...
static char *guid_printf(char *buf, const char *fmt, char *args)
{
	switch (*(++fmt)) {
		case 'p':
			return uuid_string(buf, args);
		default:
			efi_err("uuid string format error, it is not p, please fix it as p!\n");
			return NULL;
	}
	return NULL;
}

static inline char *efi_guid_unparse(efi_guid_t *guid, char *out)
{
	if (NULL == guid_printf(out, "%p", guid->b)) {
...
static int parse_gpt_header(u32 part_id, u64 header_lba, u8 *header_buf, u8 *entries_buf)
{
	...
	for (i = 0; (u32)i < header->num_partition_entries; i++) {
		...
		efi_info("%s[%d]name=%s, part_id=%d, start_sect=0x%lx, nr_sects=0x%lx, uuid=%s\n", TAG, i,
			...
			part_ptr[i].info ? part_ptr[i].info->uuid : "");
#else
...
static void pack_entries_data(void *data)
{
	...
	set_entries_crc32(efi_crc32(data, nr_parts * sizeof(gpt_entry))); //CH: D2 uses nr_parts here
}
...
int write_primary_gpt(void *data, unsigned sz)
{
	...
	pentries_write_size = (u64)((entries_num + 3) / 4) * dev->blkdev->blksz;
