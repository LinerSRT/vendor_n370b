...
/* secure boot flow */
extern int sec_func_init(unsigned long long pl_start_addr);
extern int seclib_set_oemkey(unsigned char *pubk, unsigned int size);
extern unsigned int seclib_image_check(char *img_name, unsigned char msg_en);
int sec_boot_check(int try_lock);
extern int sec_logo_check(void);
extern int sec_dtbo_check(void);
extern int sec_usbdl_verify_da(unsigned char*, unsigned int, unsigned char*, unsigned int);
extern int android_verified_boot(char *part_name, char *target);

/* get secure boot info */
extern int sec_usbdl_enabled (void);
extern int sec_get_custom_pubk(unsigned char *pubk, unsigned int size);
extern void sec_clear_pubk(void);
extern int sec_query_device_lock(int *lock_state);
extern unsigned int get_sboot_state(unsigned int *sboot);
extern unsigned int get_lock_state(unsigned int *lock_state);

/* set secure boot info */
extern unsigned *fill_atag_masp_data(unsigned int *ptr);
