...
//    start_addr = part->start_sect * dev->blkdev->blksz;
	...
	snprintf(cmdline, UART_EARLY_CONSOLE_LEN, " earlyprintk=mt6775,0x%x ", AP_UART0_BASE);
...
int mboot_android_load_bootimg(char *part_name, unsigned long addr)
{
	...
	//* read image data
	//*
	dprintf(INFO, "read the data of %s (size = 0x%x)\n", part_name, g_bimg_sz);
...
unsigned int mboot_get_img_size(char *part_name, unsigned int *size)
{
	...
	part_dev_t *dev;
	struct bootimg_hdr boot_hdr;
	unsigned page_sz = 0x800; /* used to cache page size in boot image hdr, default 2KB */
