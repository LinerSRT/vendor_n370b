...
static void _pwrap_starve_set(void)
{
	...
	WRAP_WR32(PMIC_WRAP_STARV_COUNTER_0, 0x0400);
	WRAP_WR32(PMIC_WRAP_STARV_COUNTER_1, 0x0400);
	WRAP_WR32(PMIC_WRAP_STARV_COUNTER_2, 0x0402);
	WRAP_WR32(PMIC_WRAP_STARV_COUNTER_3, 0x040e);
	WRAP_WR32(PMIC_WRAP_STARV_COUNTER_4, 0x040e);
	WRAP_WR32(PMIC_WRAP_STARV_COUNTER_5, 0x041f);
	WRAP_WR32(PMIC_WRAP_STARV_COUNTER_6, 0x041f);
	WRAP_WR32(PMIC_WRAP_STARV_COUNTER_7, 0x0402);
	WRAP_WR32(PMIC_WRAP_STARV_COUNTER_8, 0x0427);
	WRAP_WR32(PMIC_WRAP_STARV_COUNTER_9, 0x0427);
	WRAP_WR32(PMIC_WRAP_STARV_COUNTER_10, 0x0416);
	WRAP_WR32(PMIC_WRAP_STARV_COUNTER_11, 0x0416);
	WRAP_WR32(PMIC_WRAP_STARV_COUNTER_12, 0x0413);
	WRAP_WR32(PMIC_WRAP_STARV_COUNTER_13, 0x047b);
	WRAP_WR32(PMIC_WRAP_STARV_COUNTER_14, 0x047b);
	WRAP_WR32(PMIC_WRAP_STARV_COUNTER_15, 0x045b);
	WRAP_WR32(PMIC_WRAP_STARV_COUNTER_16, 0x073f);
...
static void _pwrap_enable(void)
{
	...
	WRAP_WR32(PMIC_WRAP_INT1_EN, 0xfc0001ff);
}
...
static signed int _pwrap_init_sistrobe(int dual_si_sample_settings)
{
	...
	if (found == 0) {
		result_faulty |= 0x1;
		PWRAPERR("result_faulty = %d (Valid Sampling Clock is not Found)\n", result_faulty);
	}
	...
	if (dual_si_sample_settings == 1) {
		...
		if (found == 0) {
			result_faulty |= 0x4;
			PWRAPERR("result_faulty (Dual) = %d (Valid Sampling Clock is not Found)\n", result_faulty);
		}
		if((si_en_sel == 7) && (si_ck_sel == 1)) {
			result_faulty |= 0x8;
			PWRAPERR("result_faulty (Dual) = %d (Sampling Clock is last Setting, can not Use)\n", result_faulty);
		}
		/* TINFO = "[DrvPWRAP_InitSiStrobe] Search For The Data Boundary." */
		for (si_dly = 0; si_dly < 10; si_dly++) {
...
static int __pwrap_InitSPISLV(void)
{
	...
	pwrap_write_nochk(MT6355_DRV_CON1, 0x8888); /* set IO driving strength to 4 mA */
#ifdef DUAL_PMICS
...
static signed int _pwrap_init_reg_clock(unsigned int regck_sel)
{
	...
	if (regck_sel == 1) { /* Slave Clock is 18MHz */
		...
		WRAP_WR32(PMIC_WRAP_CSLEXT_WRITE, 0x0200);
		WRAP_WR32(PMIC_WRAP_CSLEXT_READ, 0x0200);  /* for PMIC site synchronizer use*/
	} else { /*Safe Mode*/
...
#define PWRAP_GPIO_DRIVING_ADDR  0x104E0010
	WRAP_WR32(PWRAP_GPIO_DRIVING_ADDR, WRAP_RD32(PWRAP_GPIO_DRIVING_ADDR)|(0x1<<3));
	PWRAPLOG("Preloader GPIO152-155 0x104E0010 driving = 0x%x \n", WRAP_RD32(PWRAP_GPIO_DRIVING_ADDR));
#endif
	...
	PWRAPLOG("_pwrap_init_cipher write test ok\n");
	...
	*/
	...
	/* PMIC_WRAP Priority Adjustment */
	WRAP_WR32(PMIC_WRAP_PRIORITY_USER_SEL_1, 0x07060405);
	WRAP_WR32(PMIC_WRAP_ARBITER_OUT_SEL_1, 0x07060405);
...
#if 0 /* (PMIC_WRAP_PRELOADER) && (MTK_PLATFORM_MT6355) */
