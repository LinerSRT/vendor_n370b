...
static enum DSI_STATUS DSI_Reset(DISP_MODULE_ENUM module, void* cmdq)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		DISP_REG_SET_FIELD(cmdq, DSI_COM_CON_FLD_DPHY_RESET, DSI_REG_BASE[i] + DISP_REG_DSI_COM_CON, 1);
		DISP_REG_SET_FIELD(cmdq, DSI_COM_CON_FLD_DPHY_RESET, DSI_REG_BASE[i] + DISP_REG_DSI_COM_CON, 0);
	}
...
#ifdef MACH_FPGA
			/* fpga must set this */
			DISP_REG_SET(cmdq, DSI_REG_BASE[i] + DISP_REG_DSI_PHY_LCPAT, 0x55);
#endif
...
enum DSI_STATUS DSI_TXRX_Control(DISP_MODULE_ENUM module, void* cmdq, LCM_DSI_PARAMS *dsi_params)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		/* enable dsi recieve garbege data when dsi underrun,so that rdma can frame done */
		DISP_REG_SET(cmdq, DSI_REG_BASE[i] + DISP_REG_DSI_INPUT_SETTING, 1);
	}
	return DSI_STATUS_OK;
}
...
void DSI_PHY_clk_setting(DISP_MODULE_ENUM module, void* cmdq, LCM_DSI_PARAMS *dsi_params)
{
	...
	m_mipitx_impedance[0] = (struct mipitx_impedance){3, 0x10450190, 11};
	m_mipitx_impedance[1] = (struct mipitx_impedance){6, 0x10450194, 27};
	m_mipitx_impedance[2] = (struct mipitx_impedance){1, 0x10450198, 27};
...
void DSI_PHY_TIMCONFIG(DISP_MODULE_ENUM module, void* cmdq, LCM_DSI_PARAMS *dsi_params)
{
	...
	/* sync from cmm */
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		DISP_REG_SET(cmdq, DSI_REG_BASE[i] + DISP_REG_DSI_PHY_TIMCON0, 0x02000102);
		DISP_REG_SET(cmdq, DSI_REG_BASE[i] + DISP_REG_DSI_PHY_TIMCON1, 0x010a0308);
		DISP_REG_SET(cmdq, DSI_REG_BASE[i] + DISP_REG_DSI_PHY_TIMCON2, 0x02000100);
		DISP_REG_SET(cmdq, DSI_REG_BASE[i] + DISP_REG_DSI_PHY_TIMCON3, 0x00010701);
...
int ddp_dsi_trigger(DISP_MODULE_ENUM module, void* cmdq)
{
	...
	DISP_REG_SET(cmdq, DISP_REG_CONFIG_MMSYS_MISC, 0x14);
#if 0
	//dsi pattern
