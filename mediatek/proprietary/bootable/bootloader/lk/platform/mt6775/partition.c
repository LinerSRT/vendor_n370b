...
extern int ufs_lk_get_active_boot_part(u32 *active_boot_part);
#endif
part_t *mt_part_get_partition(char *name)
{
	...
	if (!strncmp(name, "PRELOADER", PART_META_INFO_NAMELEN) || !strncmp(name, "preloader", PART_META_INFO_NAMELEN)) {
		...
		tempart.nr_sects = 0x200;
#if defined(MTK_EMMC_SUPPORT)
		mmc_get_boot_part((u32 *)&tempart.part_id); // get active boot partition
#elif defined(MTK_UFS_BOOTING)
		ufs_lk_get_active_boot_part((u32 *)&tempart.part_id);
#endif
		return &tempart;
	}
...
int part_init(part_dev_t *dev)
{
	...
	partition_all[0].nr_sects = 0x400;
#if defined(MTK_EMMC_SUPPORT)
	mmc_get_boot_part((u32 *)&partition_all[0].part_id); // get active boot partition
#elif defined(MTK_UFS_BOOTING)
	ufs_lk_get_active_boot_part((u32 *)&partition_all[0].part_id);
#endif
