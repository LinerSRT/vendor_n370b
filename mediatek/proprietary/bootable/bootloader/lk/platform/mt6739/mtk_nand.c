...
/* support un-block-aligned write */
static int nand_part_alloc_buffer_align(uchar** buf_alloc, int item_size) {
	*buf_alloc = NULL;

	*buf_alloc = (uchar*)memalign(16, item_size);
	if (*buf_alloc == NULL) {
		MSG(ERR, "[%s:%d] allocate %lu bytes buffer for alignment handler fail\n",
			__FUNCTION__, __LINE__, item_size);
		return 1;
	}
	MSG(ERR, "[%s:%d] allocate %lu bytes buffer for alignment handler, addr:%x\n",
		__FUNCTION__, __LINE__, item_size, (u32)*buf_alloc);
	return 0;
}

static int nand_part_write(part_dev_t * dev, uchar * src, u64 dst, int size, int id)
{
	_dprintf("%s - dst 0x%llx size 0x%x src 0x%x 0x%x 0x%x 0x%x\n",
		__func__, dst, size, src[0], src[1], src[2], src[3]);
...
//	u32 mapped;
	...
	uchar *buf, *tmp_buf, *tmp;
	u64 part_start, aligned_start;
	u32 read_idx, read_row, u4ReadLen;
	...
	part_start    = dst &  ((u64)(BLOCK_SIZE/3) - 1);
	aligned_start = dst & ~((u64)(BLOCK_SIZE/3) - 1);

	ret = nand_part_alloc_buffer_align(&buf, BLOCK_SIZE/3);
	if (ret)
		part_start = 0;

	/* need read block data before erase */
	if (part_start) {
		MSG(ERR, "[LK] nand_part_write - block not aligned 0x%llx (0x%llx:0x%llx)\n",
			dst, part_start, aligned_start);

		read_row = aligned_start/(nand->page_size);
		u4ReadLen = 0;
		tmp_buf = (uchar*)data_buf_temp;
		for (read_idx = 0; read_idx < (BLOCK_SIZE/nand->page_size)/3; read_idx++) {
			nand_exec_read_page(nand, read_row+read_idx, u4PageSize, tmp_buf, g_kCMD.au1OOB);
			memcpy(buf + u4ReadLen, tmp_buf, u4PageSize);
			u4ReadLen += u4PageSize;
		}
	}
	...
	while (((u32)size > u4WriteLen) && (u4BlkAddr < u4BlkEnd)) {
		...
		if (!part_start) {
			...
			memcpy(buf + part_start, src, size);
			u4RowAddr = aligned_start/u4PageSize;
			u4RowEnd = ((u4RowAddr + u4PageNumPerBlock) / u4PageNumPerBlock) * u4PageNumPerBlock;
			u4WriteLen = 0;

			for (; u4RowAddr < u4RowEnd; u4RowAddr++) {
				tmp_buf = buf+u4WriteLen;
				nand_exec_write_page(nand, u4RowAddr, u4PageSize, tmp_buf, g_kCMD.au1OOB);
				u4WriteLen += u4PageSize;
			}
