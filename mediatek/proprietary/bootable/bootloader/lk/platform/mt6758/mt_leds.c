...
//rt5081 default pwm setting, used when dts didn't found
...
static void rt5081_i2c_read_byte(struct mt_i2c_t *i2c, u8 cmd)
{
	unsigned int ret = I2C_OK;
	unsigned char read_buf[2] = {cmd, 0};

	ret = i2c_write_read(i2c, read_buf, 1, 1);
	LEDS_DEBUG("[LEDS] %s: I2CW[0x%02X] = 0x%02X, code = %d\n", __func__, cmd, read_buf[0], ret);
	if (ret != I2C_OK)
		LEDS_DEBUG("[LEDS] %s: failed, code = %d\n", __func__, ret);
}
...
static void rt5081_init_bled()
{
	...
	for (i = 0; i < sizeof(rt5081_bled_settings) / sizeof(struct rt5081_bled_config_t); i++) {
		...
		rt5081_i2c_read_byte(&rt5081_bled_i2c, rt5081_bled_settings[i].addr);
	}
}
