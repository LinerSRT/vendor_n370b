...
#elif defined(MTK_UFS_BOOTING)
		ufs_lk_get_active_boot_part((u32 *)&bootpart);
#endif
...
int partition_get_index(const char * name)
{
	...
	for (i = 0; i < PART_MAX_COUNT; i++) {
		if (!partition_all[i].info)
			continue;
		if (!strcmp(name, (char *) partition_all[i].info->name)) {
...
int partition_get_type(int index, char **p_type)
{
	...
	for (i = 0; i < loops; i++) {
		if (!g_part_name_map[i].fb_name[0])
			break;
		if (!strcmp((char *)partition_all[index].info->name, g_part_name_map[i].r_name)) {
...
int part_init(part_dev_t *dev)
{
	...
	if (partition_all[0].info) {
		sprintf((char *) partition_all[0].info->name, "%s", "preloader");
	}
