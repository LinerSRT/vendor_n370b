...
// -- SIM1
#define SIM1_SIO_GPIO_ID    (GPIO15 | GPIO_MAGIC)
#define SIM1_SRST_GPIO_ID   (GPIO14 | GPIO_MAGIC)
#define SIM1_SCLK_GPIO_ID   (GPIO13 | GPIO_MAGIC)
...
#define UART2_TX_GPIO_ID (GPIO12 | GPIO_MAGIC)
#define UART2_RX_GPIO_ID (GPIO11 | GPIO_MAGIC)
#define UART3_TX_GPIO_ID (GPIO87 | GPIO_MAGIC)
#define UART3_RX_GPIO_ID (GPIO86 | GPIO_MAGIC)
#define UART4_TX_GPIO_ID (GPIO85 | GPIO_MAGIC)
#define UART4_RX_GPIO_ID (GPIO84 | GPIO_MAGIC)
...
#if 1
	{sw_pd_psmcusys_sram_1, 0, 31},
	{sw_pd_psmcusys_sram_2, 0, 31},
	{sw_pd_psmcusys_sram_3, 0, 25},
	{sw_pd_psmcusys_sram_10, 16, 31},
	{sw_pd_psmcusys_sram_11, 0, 31},
	{sw_pd_psmcusys_sram_12, 0, 25},
	{SW_PD_FOR_SRAM_1, 23, 28},
#else
	{0x160, 0, 31},
	{0x164, 0, 31},
	{0x168, 0, 31},
...
void md_uart_config(int type_id, int boot_mode)
{
	switch (type_id) {
		case AP_ONLY: // for AP only
			dprintf(CRITICAL, "md_uart_config:%d, UART1->AP_0, UART2->MD1_1, UART3->C2K_0, UART4->MD1_0\n", type_id);
			// same as dws initial setting
			mt_set_gpio_mode(UART1_TX_GPIO_ID, GPIO_MODE_01);
			mt_set_gpio_mode(UART1_RX_GPIO_ID, GPIO_MODE_01);
			mt_set_gpio_mode(UART2_TX_GPIO_ID, GPIO_MODE_02);
			mt_set_gpio_mode(UART2_RX_GPIO_ID, GPIO_MODE_02);
			mt_set_gpio_mode(UART3_TX_GPIO_ID, GPIO_MODE_06);
			mt_set_gpio_mode(UART3_RX_GPIO_ID, GPIO_MODE_06);
			mt_set_gpio_mode(UART4_TX_GPIO_ID, GPIO_MODE_06);
			mt_set_gpio_mode(UART4_RX_GPIO_ID, GPIO_MODE_06);
			break;
...
void md1_pll_init(void)
{
    ...
    *REG_MDTOP_PLLMIXED_DCXO_PLL_SETTLE_CTL = 0x02020E84; //dcxo settle time with 132T 32K	

    *REG_MDTOP_PLLMIXED_PLL_DLY_CTL0 = 0x0019000D;      // Change ISO <-> EN delay to 0x19 count
    *REG_MDTOP_PLLMIXED_BRPPLL_CTL0 = 0x80229D8A;       // 300MHz * 3*4 = 3600MHz, backup PLL for 92MD
    *REG_MDTOP_PLLMIXED_BRPPLL_CTL1 = 0x00000812;       // POSDIV = 4, DIV3_EN = 1
    *REG_MDTOP_PLLMIXED_ICCPLL_CTL0 = 0x801EEC4F;       // 268MHz * 3*4 = 3216MHz, dvfs driver use
    *REG_MDTOP_PLLMIXED_ICCPLL_CTL1 = 0x00000812;       // POSDIV = 4, DIV3_EN = 1
    ...
    *REG_MDTOP_PLLMIXED_MDMCUPLL_CTL0 = 0x80229D8A;     // 900MHz * 4
 /*----------------------------------------------------------------------------------------
...
void c2k_boot(int boot_mode)
{
	...
	ccci_write32(INFRACFG_AO_BASE, INFRA_TOPAXI_PROTECTEN_1_CLR,
			ccci_read32(INFRACFG_AO_BASE, INFRA_TOPAXI_PROTECTEN_1_SET) & 0x03C00000);
