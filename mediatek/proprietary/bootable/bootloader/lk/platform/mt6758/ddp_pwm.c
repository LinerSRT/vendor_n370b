...
#ifndef CLK_CFG_1
#define CLK_CFG_1 0x10210110
#endif

#ifndef CLK_CFG_1_SET
#define CLK_CFG_1_SET 0x10210114
#endif

#ifndef CLK_CFG_1_CLR
#define CLK_CFG_1_CLR 0x10210118
#endif
	...
	if (cust_led_list) {
		...
		if ((strcmp(cust->name,"lcd-backlight") == 0) && (cust->mode == MT65XX_LED_MODE_CUST_BLS_PWM)) {
			...
			if (config_data->clock_source >= 0 && config_data->clock_source <= 3) {
				unsigned int reg_val = PWM_REG_GET(CLK_CFG_1);
				PWM_REG_SET(CLK_CFG_1_CLR, 0x30000);
				PWM_REG_SET(CLK_CFG_1_SET, (0x30000 & (config_data->clock_source << 16)));
				PWM_DBG("disp_pwm_init : CLK_CFG_1 0x%x => 0x%x", reg_val, PWM_REG_GET(CLK_CFG_1));
			}
