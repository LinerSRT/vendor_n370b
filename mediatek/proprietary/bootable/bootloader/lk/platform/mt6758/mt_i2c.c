...
#endif
	//I2CLOG("tmo = 0x%x\n", tmo);
	if (!(tmo ==0) && (!i2c->dma_en) && (i2c->op == I2C_MASTER_RD || i2c->op == I2C_MASTER_WRRD) ) {
			/*only read mode or write_read mode and fifo mode need to get data*/
			data_size = (i2c_readl(i2c, OFFSET_FIFO_STAT) >> 4) & 0x000F;
			read_len = i2c->op == I2C_MASTER_RD ? i2c->msg_len & 0x000f : (i2c->msg_len >> 16) & 0x000f;
			if (data_size > read_len) {
				I2CERR("i2c data_size is larger than read_len. data_size is %d and read_len is %d", data_size, read_len);
				TRANSFER_ERROR=TRUE;
			}
	}
