...
#ifdef MTK_CHARGER_INTERFACE
	if (!primary_mchr) {
		dprintf(CRITICAL, "%s: primary charger not registered\n", __func__);
		return;
	}
...
void check_bat_protect_status(void)
{
	...
	while (bat_val < BATTERY_LOWVOL_THRESOLD) {
		...
		if (chr_volt > V_CHARGER_MAX) {
			dprintf(CRITICAL,
				"[BATTERY] charger voltage is too high :%d , threshold is %d !\n",
		...
		dprintf(CRITICAL, "[%s]: check VBAT=%d mV with %d mV, start charging...\n",
			__func__, bat_val, BATTERY_LOWVOL_THRESOLD);
		...
		for (i = 0; i < 10; i++) {
			...
			if (current < 100 && chr_volt < 4400) {
				cnt++;
				dprintf(CRITICAL,
					"[BATTERY] charging current=%d charger volt=%d\n\r",
					current, chr_volt);
			} else {
				dprintf(CRITICAL,
					"[BATTERY] charging current=%d charger volt=%d\n\r",
		...
		if (cnt >= 8) {
			...
			dprintf(CRITICAL,
				"[BATTERY] charging current and charger volt too low !!\n\r", cnt);
			...
			while (1) {
				dprintf(CRITICAL,
					"If you see the log, please check with RTC power off API\n\r");
			}
		}
		mdelay(50);
#endif
...
void check_sw_ocv(int bat_vol)
{
	...
	dprintf(CRITICAL,
		"[check_sw_ocv]%d ptim[%d %d] fg_swocv[%d %d] boot_vbat:%d shutdowntime:%d vbat:%d\n",
...
void mt65xx_bat_init(void)
{
	...
	if (g_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT
	    && (pmic_get_register_value(PMIC_PWRKEY_DEB) == 0)) {
...
#endif
	{
		if (g_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT && upmu_is_chr_det() == KAL_TRUE) {
			...
			if (bat_vol < BAT_LV_NO_CHR) {
				dprintf(CRITICAL,
					"[BATTERY] battery voltage(%dmV) <= %d ! Can not Boot Linux Kernel !!\n\r",
					bat_vol, BAT_LV_NO_CHR);
#ifndef NO_POWER_OFF
				mt6575_power_off();
#endif
				while (1) {
	...
	dprintf(INFO, "[mt65xx_bat_init] end VBAT=%d c_th:%d,noc_th:%d mV is_charging:%d\n",
		bat_vol, BATTERY_LOWVOL_THRESOLD, BAT_LV_NO_CHR, is_charging);
...
int get_rac_val(void)
{
	...
	do {
		...
		dprintf(INFO,
			"[5,Calculate Rac] volt_1=%d,volt_2=%d,curr_1=%d,curr_2=%d,rac_cal=%d,ret=%d,retry_count=%d,0x%x,0x%x\n",
			volt_1, volt_2, curr_1, curr_2, rac_cal, ret, retry_count,
			upmu_get_reg_value(0x3310), upmu_get_reg_value(0x3370));
...
void get_dlpt_imix_r(void)
{
	...
	dprintf(CRITICAL, "[dlpt_R] %d,%d,%d,%d,%d [%d:%d:%d]%d\n", rac_val[0], rac_val[1],
		rac_val[2], rac_val[3], rac_val[4], min, max, validcnt, imix_r);
