...
extern int 	spm_mtcmos_ctrl_md1(int);
		...
		for (tags = (void *)BOOT_ARGUMENT_LOCATION; tags->hdr.size; tags = boot_tag_next(tags)) {
			switch (tags->hdr.tag) {
				...
				case BOOT_TAG_GZ_INFO:
					g_boot_arg->gz_addr = tags->u.gz_info.gz_addr;
					break;
#endif
				default:
					break;
...
void disable_scp_hw(void)
{
	...
	*/
	...
	DRV_WriteReg32(SCP_SRAM_PDN, 0xFFFFFF); /*turn off scp sram*/
	DRV_WriteReg32(SCP_CLK_SEL, 0x1);    /*turn off scp clock*/
	DRV_WriteReg32(SCP_CLK_EN, 0x0);
	DRV_WriteReg32(SCP_CFGREG_SW_RSTN, 0x0);

	dprintf(CRITICAL, "DISABLE SCP: 0x%x, 0x%x, 0x%x, 0x%x\n",
			DRV_Reg32(SCP_SRAM_PDN),
			DRV_Reg32(SCP_CLK_SEL),
			DRV_Reg32(SCP_CLK_EN),
			DRV_Reg32(SCP_CFGREG_SW_RSTN));
}
...
#if (MTK_DUAL_DISPLAY_SUPPORT == 2)
	g_ext_fb_base = mblock_reserve_ext(&g_boot_arg->mblock_info, g_ext_fb_size, 0x10000, 0x80000000, 0, "ext_framebuffer");
#endif
#endif
...
#if (MTK_DUAL_DISPLAY_SUPPORT == 2)
	if (!g_ext_fb_base) {
		dprintf(CRITICAL, "reserve ext_framebuffer failed\n");
	}
#endif
#endif
