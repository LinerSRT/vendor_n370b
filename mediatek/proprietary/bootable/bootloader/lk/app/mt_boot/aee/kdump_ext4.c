...
static inline uint16_t paf_version_extract(uint8_t pafinfo[MRDUMP_PAF_SIZE])
{
	return pafinfo[0] + (pafinfo[1] << 8);
}

static void dump_paf_info(uint8_t InfoLBA[MRDUMP_PAF_SIZE], const char *msg, ...)
{
	va_list ap;
	va_start(ap, msg);
	voprintf('E', msg, ap);
	...
	voprintf_info(" pafile ver %d info-lba %d addr-lba %d alloc-size %lld coredump-size %lld crc %08x\n",
		      paf_version, paf_info_lba, paf_addr_lba, paf_alloc_size, paf_coredump_size, paf_crc32);
}
...
{
	uint8_t InfoLBA[MRDUMP_PAF_SIZE];
	unsigned int mycrc;
