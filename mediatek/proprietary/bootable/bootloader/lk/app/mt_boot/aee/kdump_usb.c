...
static bool usb_data_transfer(struct mrdump_usb_handle *handle, int length)
{
	...
	len = usb_write_with_timeout(cmd, strlen(cmd), CON_TIMEOUT);
	if (len > 0) {

		/* receive CTS */
		memset(cmd, 0, MAX_RSP_SIZE);
		len = usb_read_with_timeout(cmd, MAX_RSP_SIZE, CON_TIMEOUT);
		if ((len == (int)strlen(USB_CTS))&&
			...
			if (len > 0) {
				...
				len = usb_read_with_timeout(cmd, sizeof(cmd), CON_TIMEOUT);
				if ((len == (int)strlen(USB_FIN)) &&
				    (!strncmp(cmd, USB_FIN, strlen(USB_FIN)))) {
					return true;
				} else {
					voprintf_error("%s: failed to get FIN.\n", __func__);
				}
